...
class BotDataBase: 
    def __init__(self, **args):
        self.user = args.get('user', 'hopa')
        self.password = args.get('password', 'admin')
        self.port = args.get('port', 5432)
        self.dbname = args.get('dbname', 'GluuBot')
        self.host = args.get('host', 'localhost')
        self.connection = None
        self.nlu_json = args.get('nlu_json',{ "version": "3.1","nlu":[]})
        self.domain_json = args.get('domain_json',{ "version": "3.1","intents":[],"responses":{},"session_config": {"session_expiration_time": 60, "carry_over_slots_to_new_session": True}})
        self.rules_json = args.get('rules_json',{ "version": "3.1","rules":[],})
    ...
    def get_certainValue(self,select='*',where='',equal=''):
        ...
        if where != '' :
            query=(f'''SELECT {select} FROM intent WHERE intent.{where}= '{equal}' ''')
            return self.get_response(query)[0][0]
        else:
    ...
    def update_value(self,value,new_value,where,where_value):
        query= (f'''UPDATE intent SET {value} = '{new_value}' WHERE {where} = {where_value} ''')
    ...
    def get_domainData(self,directory_data):
        ...
        with open(f"{directory_data}/domain.yaml", "w") as yaml_out:
            self.converter.get_yaml(json_data=self.domain_json,yaml_out=yaml_out)
    ...
    def get_rulesData(self,directory_data):
        ...
        with open(f"{directory_data}/rules.yaml", "w") as yaml_out:
            self.converter.get_yaml(json_data=self.rules_json,yaml_out=yaml_out)
